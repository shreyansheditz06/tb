...
client.on("interactionCreate", async (interaction) => {
    if (interaction.isButton()) {
        ...
        if (buttonID === "close-ticket-opn") {
            ...
            return interaction.channel.send({
                embeds: [
                    new MessageEmbed()
                        .setTitle("Ticket System ✅")
                        .setDescription(
                            client.languages.__mf("buttons.close.messages.closed_ticket", {
            ...
            // Rename the channel to claimed-by-<claimer>
            const claimerTag = interaction.user.tag.replace(/[^a-zA-Z0-9]/g, "-").toLowerCase();
            const newChannelName = `claimed-by-${claimerTag}`;
            ...
            interaction.channel.send({
                embeds: [
                    new MessageEmbed()
                        .setTitle("Ticket System ✅")
                        .setDescription(
                            client.languages.__mf("buttons.claim.messages.claimed_ticket", {
                                user_mention: `<@${interaction.user.id}>`,
                                user_id: interaction.user.id,
                            })
                        )
            ...
            interaction.channel.send({
                embeds: [
                    new MessageEmbed()
                        .setTitle("Ticket System ✅")
                        .setDescription(
                            client.languages.__mf("buttons.open.messages.ticket_opened", {
                                user_mention: `<@${interaction.user.id}>`,
                                user_id: interaction.user.id,
            ...
            interaction.channel.send({
                embeds: [
                    new MessageEmbed()
                        .setTitle("Ticket System ✅")
                        .setDescription(
                            client.languages.__mf("buttons.delete.messages.deleting_ticket", {
                                time: "5",
                            })
                        )
                        .setColor("RED"),
